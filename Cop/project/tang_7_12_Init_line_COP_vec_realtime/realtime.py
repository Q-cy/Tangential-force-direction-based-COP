...
class RealTimePlot:
    ...
    def set_data(self, adc_angle, adc_mag, force_angle, force_mag, diff_frame, total_pressure_sum, force_total_mag,
                 cop_x, cop_y, base_cop_x, base_cop_y, delta_cop_x, delta_cop_y, raw_fx, raw_fy): # 新增 raw_fx, raw_fy
        with self.lock:
            ...
            self.adc_mag_history.append(adc_mag) # 仍然需要，用于绘制全程总幅值曲线
            self.raw_force_mag_history.append(force_total_mag) # 仍然需要

            # 新增：追加 CoP 偏移分量 (PZT Fx/Fy)
            self.adc_dx_history.append(delta_cop_x)
            self.adc_dy_history.append(delta_cop_y)

            # 新增：追加力传感器分量 (Force Fx/Fy)
    ...
    def update_all(self, frame):
        ...
        self.update_adc_components() # 调用新的更新函数
        self.update_force_components() # 调用新的更新函数
        self.update_error()
