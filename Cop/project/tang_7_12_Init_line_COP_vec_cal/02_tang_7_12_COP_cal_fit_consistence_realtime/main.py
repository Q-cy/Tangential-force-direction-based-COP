...
MAIN_CONSISTENCE_COEFFS_PATH = "/home/qcy/Project/data/2.PZT_tangential/weight/png/consistence_coeffs.npy"
MAIN_USE_CONSISTENCE = True                  # 是否启用一致性校准（CSV始终同时保存原始+校准）
MAIN_DISPLAY_CALIBRATED = True               # True=热力图显示校准后数据, False=显示原始数据
...
def data_loop():
    ...
    # 加载一致性校准系数
    consistence_coeffs = None
    if MAIN_USE_CONSISTENCE and os.path.exists(MAIN_CONSISTENCE_COEFFS_PATH):
        try:
            consistence_coeffs = consistence.load_coeffs(MAIN_CONSISTENCE_COEFFS_PATH)
            print(f"📐 一致性系数已加载: {MAIN_CONSISTENCE_COEFFS_PATH}")
        except Exception as e:
            print(f"⚠️ 一致性系数加载失败: {e}")
    ...
    while not g_main_stop_flag.is_set():
        ...
        if press_item is not None:
            raw_ch_data = press_item["data"]
            # 一致性校准
            if consistence_coeffs is not None:
                cal_ch_data = consistence.apply_consistence(raw_ch_data, consistence_coeffs)
            else:
                cal_ch_data = raw_ch_data
            # 显示数据源
            display_ch_data = cal_ch_data if MAIN_DISPLAY_CALIBRATED else raw_ch_data
            # COP 计算仍用原始数据
            cop_res = COP.compute_pressure_direction(raw_ch_data)
            base_sub_arr = np.array(display_ch_data)
            ...
            total_press_val = np.sum(raw_ch_data)
