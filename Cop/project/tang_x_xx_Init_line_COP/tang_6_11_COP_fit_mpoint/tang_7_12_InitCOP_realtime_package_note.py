...
class PRSensorAngle:
    """压阻传感器：11×6（默认）PZT 阵列的 CoP 角度估计"""

    def __init__(self, rows: int = 11, cols: int = 6,
        ...
        :param rows: 传感器阵列行数（默认 11）。
        :param cols: 传感器阵列列数（默认 6），输入 ADC 序列长度 = rows * cols。
        ...
        for reg in regions_info:
            for hi in merged:                                 # merged 内按 peak 降序
                if (reg['peak'] < self._region_peak_ratio * hi['peak']
                    ...
                    # 对合并后 coords 重算特征（region ≤ 66 cells, 重算代价可忽略）
