...
from data import (PressureSensor, SixAxisForceSensor, TimestampedBuffer, match_closest,
                  ...
                  PRESSURE_SENSOR_PORT, PRESSURE_SENSOR_PORT_2, FORCE_SENSOR_PORT)
...
MAIN_PRESSURE_PORTS = [PRESSURE_SENSOR_PORT, PRESSURE_SENSOR_PORT_2]  # 两路压阻串口
MAIN_FIT_PATHS = [fit_coefs_path, os.path.join(BASE_DIR, "fit_coefs_2.bin")]  # 每路 fit 模型
MAIN_SAVE_SUBDIRS = ["sensor1", "sensor2"]   # 每路独立保存子目录
...
# ===================== 单路采集管线 =====================
def run_pipeline(plot, press_port, tag, fit_path, save_dir, sensor_force, buf_force, has_force):
    has_press = True           # 是否有压力传感器

    try:
        sensor_press = PressureSensor(port=press_port)
        buf_press = TimestampedBuffer(500)
        thread_press = PressureThread(sensor_press, buf_press)
        thread_press.start()
        print(f"✅ [{tag}] 压力传感器就绪: {press_port}")
    except Exception as e:
        has_press = False
        buf_press = None
        print(f"⚠️ [{tag}] 压力传感器未连接: {e}")

    if not has_press:
        print(f"❌ [{tag}] 无压力传感器，退出")
        return

    # 自动获取CSV文件路径（每路独立子目录, 避免并发文件名冲突）
    csv_path = auto_get_csv_path(save_dir)
    # 初始化CSV文件（写入表头）
    ...
    if os.path.exists(fit_path):
        try:
            fit_type, _, fit_params_list, fit_split_sign = load_coefs(fit_path)
            type_summary = ", ".join(f"{p[1]}{'(split)' if p[2] else ''}" for p in fit_params_list)
            print(f"📐 [{tag}] fit模型已加载: {fit_path} (outputs: {type_summary})")
        except Exception as e:
            print(f"⚠️ [{tag}] fit模型加载失败: {e}")
            fit_split_sign = False
    else:
        print(f"💡 [{tag}] 未找到 fit 模型文件")
    ...
    cop_sensor = PRSensorAngle(rows=PRESSURE_ROWS, cols=PRESSURE_COLS)  # 每帧唯一推进 CoP 状态的入口
    ...
    while not g_main_stop_flag.is_set():
        ...
        if press_item is not None:
            ...
            print(f"[{tag}] frame {rel_time_ms}ms: angle={pzt_angle_deg:.1f}°, "
    ...
    # 资源清理: 关闭压阻串口 (close 内含线程 join, 由 try 兜底); 六维力由 main 统一关闭
    try:
        sensor_press.close()
    except Exception:
        pass
    ...
    if row_count <= 0:
        os.remove(csv_path)
        print(f"⚠️ [{tag}] 无有效数据，CSV 已删除")
    else:
        print(f"✅ [{tag}] CSV已关闭（{row_count} 行）")
...
def main():
    ...
    # 共享的六维力传感器（两路压阻共同参考; 只在 main 里打开一次）
    has_force = True
    sensor_force = None
    buf_force = None
    try:
        sensor_force = SixAxisForceSensor(port=FORCE_SENSOR_PORT)
    ...
    # 两个压阻传感器 → 两个独立实时窗口, 各自一条采集管线
    plots = [RealTimePlot(title="PZT Sensor 1"), RealTimePlot(title="PZT Sensor 2")]
    threads = []
    for plot, port, fit_path, subdir in zip(plots, MAIN_PRESSURE_PORTS, MAIN_FIT_PATHS, MAIN_SAVE_SUBDIRS):
        save_dir = os.path.join(MAIN_SAVE_DIR, subdir)
        os.makedirs(save_dir, exist_ok=True)
        t = threading.Thread(target=run_pipeline,
                             args=(plot, port, subdir, fit_path, save_dir, sensor_force, buf_force, has_force))
        t.start()
        threads.append(t)
    ...
    try:
        ...
        for t in threads:
            t.join(timeout=2)
        if has_force and sensor_force:
            try:
                sensor_force.close()
            except Exception:
                pass
        for plot, subdir in zip(plots, MAIN_SAVE_SUBDIRS):
            try:
                plot.plot_full_analysis(os.path.join(MAIN_SAVE_DIR, subdir))
            except Exception:
                pass
