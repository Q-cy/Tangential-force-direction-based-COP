...
TABLE_CSV_HEADER = [  # CSV 文件表头（66通道 + 时间戳 + 力/角度/标定数据）
    "timestamp", "rel_ms", "adc_sum",
    # ch1 ~ ch66
    *(f"ch{i}" for i in range(1, 67)),
    # 力传感器数据
    "Fx", "Fy", "Fz", "Mx", "My", "Mz",
    # 时间戳相关
    "press_t", "force_t", "dt",
...
def build_csv_row(
    ...
    csv_row = [
        ...
        adc_sum,                 # adc_sum：ADC 66通道之和
        *ch_data,                # ch1~ch66：压力传感器66通道数据
