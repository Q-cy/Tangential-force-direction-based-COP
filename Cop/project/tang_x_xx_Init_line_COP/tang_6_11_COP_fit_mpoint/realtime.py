...
from data import PRESSURE_ROWS, PRESSURE_COLS, PRESSURE_CELLS
...
class CellGridItem(pg.GraphicsObject):
    """66 个独立色块 + 数值文字，复现 matplotlib table 效果"""
    def __init__(self, rows=PRESSURE_ROWS, cols=PRESSURE_COLS):
...
class RealTimePlot:
    def __init__(self, title="PZT RealTime"):
        self.title = title
        self.rows, self.cols = PRESSURE_ROWS, PRESSURE_COLS
    ...
    def init_defaults(self):
        ...
        self._press_table_arr = np.zeros((PRESSURE_ROWS, PRESSURE_COLS))  # 压力表数据(11×6)
        ...
        self._gradient_arr = np.zeros((PRESSURE_ROWS, PRESSURE_COLS, 2), dtype=np.float32)  # 压力梯度(每帧由 main 传入)
        self._contact_init = False
        self._pzt_table_angle_deg = None     # Pressure Table 专用角度（invertY 视图）
        self._region_mask = np.zeros((PRESSURE_ROWS, PRESSURE_COLS), dtype=np.int32)   # per-region 着色掩码
    ...
    def build_layout(self):
        ...
        self.p_table.setXRange(-0.5, PRESSURE_COLS - 0.5); self.p_table.setYRange(-0.5, PRESSURE_ROWS - 0.5)
        self.p_table.getViewBox().setBackgroundColor('w')
        self.p_table.getViewBox().setBorder(pg.mkPen(width=0))
        # CellGridItem — 66 个独立色块 + 网格线（网格线在 paint() 中绘制）
        self._cell_grid = CellGridItem(PRESSURE_ROWS, PRESSURE_COLS)
        self.p_table.addItem(self._cell_grid)
        # 数值文字
        self._cell_txts = []
        for r in range(PRESSURE_ROWS):
            row_t = []
            for c in range(PRESSURE_COLS):
        ...
        self.p_grad.setXRange(-0.5, PRESSURE_COLS - 0.5); self.p_grad.setYRange(-0.5, PRESSURE_ROWS - 0.5)
        self.p_grad.getViewBox().setBackgroundColor('w')
        self.p_grad.getViewBox().setBorder(pg.mkPen(width=0))
        self._grid_lines = GridLinesItem(PRESSURE_ROWS, PRESSURE_COLS)
        self.p_grad.addItem(self._grid_lines)
        self._g_lines = []
        self._g_heads = []
        for _ in range(PRESSURE_CELLS):
    ...
    def update_all(self):
        ...
        if contact_init:
            ...
            for row_idx in range(PRESSURE_ROWS):
                for col_idx in range(PRESSURE_COLS):
            ...
            for grad_idx, (grad_ln, grad_dot) in enumerate(zip(self._g_lines, self._g_heads)):
                grad_row, grad_col = divmod(grad_idx, PRESSURE_COLS)
            ...
            # CoP 未确定：清空两张表
            self._cell_grid.set_data(np.zeros((PRESSURE_ROWS, PRESSURE_COLS)), 1.0)
            self._cell_grid.set_regions(np.zeros((PRESSURE_ROWS, PRESSURE_COLS), dtype=np.int32), [])
            for row_idx in range(PRESSURE_ROWS):
                for col_idx in range(PRESSURE_COLS):
