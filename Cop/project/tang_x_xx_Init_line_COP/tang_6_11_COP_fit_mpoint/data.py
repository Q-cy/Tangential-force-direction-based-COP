...
# 固定串口路径
PRESSURE_SENSOR_PORT = "/dev/ttyUSB0"   # 压阻 1
PRESSURE_SENSOR_PORT_2 = "/dev/ttyUSB1" # 压阻 2
FORCE_SENSOR_PORT = "/dev/ttyUSB2"      # 六维力（两路压阻共用）

# 压阻阵列维度 (11×6 = 66 cells)
PRESSURE_ROWS = 11
PRESSURE_COLS = 6
PRESSURE_CELLS = 66
PRESSURE_PAYLOAD_LEN = 132   # 66 cells × 2 字节 (uint16 LE)
PRESSURE_FRAME_LEN = 147     # 14B header + 132B payload + 1B CRC
...
# 请求帧静态前缀: 起始符 | 数据长度 | 设备地址 | 预留 | 功能码 | 起始地址
_CMD_PREFIX = bytes([0x55, 0xAA, 0x09, 0x00, 0x34, 0x00,
                     0xFB, 0x00, 0x1C, 0x00, 0x00])

class PressureSensor:
    # 请求帧: _CMD_PREFIX | 读取长度(2B LE) | CRC-8-ITU
    CMD_BYTES = _CMD_PREFIX + struct.pack("<H", PRESSURE_PAYLOAD_LEN) \
                + bytes([crc8_itu(_CMD_PREFIX + struct.pack("<H", PRESSURE_PAYLOAD_LEN))])
    FRAME_LEN = PRESSURE_FRAME_LEN  # 14B header + 132B payload + 1B status
    MAX_RX_BUF = 4096            # 缓存区大小

    def __init__(self, port=PRESSURE_SENSOR_PORT, baudrate=DATA_BAUDRATE_PRESS):
        self.ser = None
        self.port = port
...
class SixAxisForceSensor:
    ...
    def __init__(self, port=FORCE_SENSOR_PORT, baudrate=DATA_BAUDRATE_FORCE):
        self.ser = None
        self.port = port
        self.baudrate = baudrate
