...
def data_loop():
    ...
    # 压力传感器零点校准（多帧平均，与力传感器 calibrate_zero 同理）
    time.sleep(0.3)  # 等待缓冲区积累数据
    calib_frames = []
    for _ in range(20):
        item = buf_press.get_latest()
        if item:
            calib_frames.append(np.array(item["data"], dtype=np.float32))
        time.sleep(0.05)
    if len(calib_frames) >= 5:
        avg_baseline = np.mean(calib_frames, axis=0)
        COP.set_baseline(avg_baseline)
        print("✅ 压力传感器基线校准完成")
    else:
        print("⚠️ 压力传感器基线校准采样不足，使用自动捕获")
    ...
    while not stop_event.is_set():
        ...
        # 数据新鲜度检查：超过 150ms 未更新则视为传感器失联，清空显示
        data_age = time.perf_counter() - press_data_item["t"]
        if data_age > 0.15:
            empty_frame = np.zeros(84, dtype=np.float32)
            plot.set_data(0.0, 0.0, 0.0, 0.0,
                          empty_frame, 0.0, 0.0,
        ...
        regions = cop_res['regions']

        if not regions:
            time.sleep(0.001)
            continue

        # 主COP（最大压力区域）用于现有绘图通道
        primary = regions[0]
        cx, cy = primary['cop_x'], primary['cop_y']
        dx, dy = primary['delta_cop_x'], primary['delta_cop_y']
        bx, by = primary['base_cop_x'], primary['base_cop_y']
        ...
        # 为每个区域计算角度/幅值，附加到 region dict 中
        for r in regions:
            ang, mag = angle.compute_PZT_angle(r['delta_cop_x'], r['delta_cop_y'])
            r['angle'] = ang
            r['mag'] = mag
