...
class Stage1ResourceTests(unittest.TestCase):
    ...
    def test_missing_consistence_coefficients_fail_before_csv_creation(self):
        """一致性系数失败时，压力资源已关闭且不会留下空 CSV。"""
        class FakePressure:
            def __init__(self):
                self.closed = False
        ...
        with self.subTest(case="missing NPZ"):
            with tempfile.TemporaryDirectory() as directory:
                missing = Path(directory) / "missing-consistence.npz"
                config = FullApplicationConfig(
                    save_dir=directory,
                ...
                with self.assertRaises(FileNotFoundError):
                    session.start()
                self.assertTrue(pressure.closed)
