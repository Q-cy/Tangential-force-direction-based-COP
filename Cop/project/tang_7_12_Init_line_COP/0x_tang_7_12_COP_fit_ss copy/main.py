...
from preslip import PreSlipDetector
...
def data_loop():
    ...
    preslip = PreSlipDetector()  # 预滑动检测器（CoP FFT，与 origin-locking 并行独立）；固定阈值 feature > 15
    preslip.bind(cop_sensor)
    ...
    _last_status_print_t = 0.0             # 状态打印共享节流: 上次打印时间
    STATUS_PRINT_INTERVAL_S = 0.1         # 状态打印共享周期 (秒), 10Hz
    preslip_count_val = 0                 # 累计预滑动次数（启动初始化为 0，运行时仅由 preslip.update() 刷新，不随主循环重置）

    while not g_main_stop_flag.is_set():
        loop_start_s = time.perf_counter()
        rel_time_ms = int((loop_start_s - start_time_s) * 1000)

        # ---- 预滑动输出占位（每帧重置，未触发时为 None → CSV 写 NaN/0）----
        # 注意: preslip_count_val 不在此重置——它是程序启动后的累计计数
        preslip_amp_val = preslip_flag_val = None
        preslip_dx_val = preslip_dy_val = None
        ...
        if press_item is not None:
            ...
            # frame 行打印已合并到主循环末尾的 10Hz 共享节流块

            # ---- 预滑动检测（60ms 门控在 detector 内部；触发新事件时终端打印）----
            preslip_out = preslip.update(base_sub_arr, time.perf_counter())
            if preslip_out is not None:
                preslip_amp_val = preslip_out["feature"]
                preslip_flag_val = 1 if preslip_out["is_pre_slip"] else 0
                preslip_dx_val = preslip_out["drift_xy"][0]
                preslip_dy_val = preslip_out["drift_xy"][1]
                preslip_count_val = preslip_out["slip_count"]
                if preslip_out["is_new_event"]:
                    amp_str = f"{preslip_amp_val:>6.3f}" if preslip_amp_val is not None else "   nan"
                    print(f"⚠️ 预滑动 #{preslip_count_val:>5d} at {rel_time_ms:>6d}ms, feature={amp_str}")
        ...
        csv_row = build_csv_row(
            ...
            preslip_amp=preslip_amp_val,
            preslip_flag=preslip_flag_val,
            preslip_dx=preslip_dx_val,
            preslip_dy=preslip_dy_val,
            preslip_count=preslip_count_val,
        )
        ...
        # ---- 10Hz 共享节流打印（frame + preslip + slip 同步）----
        now_p = time.perf_counter()
        if now_p - _last_status_print_t >= STATUS_PRINT_INTERVAL_S:
            print(f"frame {rel_time_ms:>6d}ms: angle={pzt_angle_deg:>6.1f}° "
                  f"dx={cop_delta_x:>+7.2f} dy={cop_delta_y:>+7.2f} "
                  f"cop=({cop_curr_x:>5.2f},{cop_curr_y:>5.2f})")
            # feature/flag/drift 仍用每帧重置的缓存（正常变化）；count 用粘性变量
            amp_str = f"{preslip_amp_val:>7.3f}" if preslip_amp_val is not None else "    nan"
            flag_str = str(preslip_flag_val if preslip_flag_val is not None else 0)
            count_str = str(preslip_count_val)
            dx_str = f"{preslip_dx_val:>+7.3f}" if preslip_dx_val is not None else "   nan"
            dy_str = f"{preslip_dy_val:>+7.3f}" if preslip_dy_val is not None else "   nan"
            print(f"[preslip] t={rel_time_ms:>6d}ms feature={amp_str} "
                  f"flag={flag_str:>2} count={count_str:>5} "
                  f"drift=({dx_str},{dy_str})")
            print(f"[slip] t={rel_time_ms:>6d}ms count={count_str:>5}")
            _last_status_print_t = now_p
