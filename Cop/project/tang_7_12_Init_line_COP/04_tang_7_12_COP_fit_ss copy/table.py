...
TABLE_CSV_HEADER = [  # CSV 文件表头（84通道 + 时间戳 + 力/角度/标定数据）
    ...
    "CoP_state",
    # 预滑动检测（preslip.py）
    "preslip_amp", "preslip_flag", "preslip_dx", "preslip_dy", "preslip_count"
]
...
def build_csv_row(
    ...
    preslip_amp: float = None,     # CoP ρ FFT 低频段最大幅值（未触发时 None）
    preslip_flag: int = None,      # 预滑动标志 0/1（未触发时 None）
    preslip_dx: float = None,      # 漂移方向 x（cell）
    preslip_dy: float = None,      # 漂移方向 y（cell）
    preslip_count: int = None,     # 累计预滑动事件数
) -> list:
    """
    构造符合表头格式的CSV行数据
    :return: 完整的CSV行列表
    ...
    csv_row = [
        ...
        *force_data,             # Fx,Fy,Fz,Mx,My,Mz：力传感器数据
        ...
        cop_state,
        preslip_amp if preslip_amp is not None else float('nan'),
        preslip_flag if preslip_flag is not None else 0,
        preslip_dx if preslip_dx is not None else float('nan'),
        preslip_dy if preslip_dy is not None else float('nan'),
        preslip_count if preslip_count is not None else 0,
    ]
    return csv_row
