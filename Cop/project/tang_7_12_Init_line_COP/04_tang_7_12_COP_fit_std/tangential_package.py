...
class FitCalibrationModel:
    ...
    def from_path(cls, path: str):
        ...
        try:
            from fit import load_coefs

            fit_type, _, params_list, split_sign = load_coefs(path)
    ...
    def predict(self, dx: float, dy: float, total: float,
                cal_dim: str = "3D") -> tuple[float, float, float]:
        if not self.available:
            return (float("nan"),) * 3
        from fit import apply_predict_multi

        inputs = [dx, dy]
        if cal_dim == "3D":
            inputs.append(total)
        values = list(apply_predict_multi(
...
class TangentialFrameProcessor:
    """复用 ``PRSensorAngle`` 和 ``fit.py`` 处理一个84通道压力帧。"""
    ...
    def __init__(self, sensor=None, processor=None, sensor_factory=None,
        ...
        if sensor is None:
            if sensor_factory is None:
                from data import PressureSensor
        ...
        if processor is None:
            if model_path is None:
                model_path = os.path.join(
                    os.path.dirname(os.path.abspath(__file__)), "fit_coefs.bin"
                )
