...
示例读取压力帧并直接 `print(frame)`。终端输出格式由 Python 的对象表示决定，用户可以在自己的程序中按需要格式化 `frame.raw`、统计值或字段。
...
<td style="white-space:normal"><code>process_frame(raw, frame=None)</code> 和处理配置</td>
...
公开采集和处理接口始终只返回 `TangentialFrame`，不会要求用户接触内部详细样本。`raw` 是长度为 84 的一维 ADC 数组；需要矩阵时可以在用户代码中执行 `frame.raw.reshape(12, 7)`。
...
<td style="white-space:normal">保存原始压力通道</td>
...
raw = np.zeros(84, dtype=np.uint16)
frame = processor.process_frame(raw)
print(frame)
```
