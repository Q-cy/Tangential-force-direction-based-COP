...
class PressureThread(threading.Thread):
    ...
    def run(self):
        ...
        Side Effects:
            ...
            ``raw_data``、请求序号和时序元数据的帧；未处理异常保存到 ``error``。
        """
        try:
            while not self.stop_event.is_set():
                ...
                try:
                    data = np.asarray(
                        self.sensor.decode(frame["payload"]), dtype=np.float64
                    )
                    self.buffer.append({
                        "t": frame["rx_t"],
                        "raw_data": data,
...
class FullAcquisitionSession:
    ...
    def _process_pressure(self, press_item):
        ...
        Args:
            press_item (dict): 压力缓存帧，必须含 ``t`` 和 ``raw_data``，可含
        ...
        # 这里不能再次执行 CoP、滑移或标定。
        sample = self.sample_processor._process_sample(press_item["raw_data"], metadata)
