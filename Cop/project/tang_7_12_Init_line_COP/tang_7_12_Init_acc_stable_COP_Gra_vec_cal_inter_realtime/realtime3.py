...
class RealTimePlot:
    ...
    def init_defaults(self):
        ...
        self._cop_angle_raw = 0.0           # 纯COP位移角度
        self._cop_mag_raw = 0.0             # 纯COP位移幅值
        self._grad_angle_raw = 0.0          # 纯梯度角度
        self._grad_mag_raw = 0.0            # 纯梯度幅值
    ...
    def build_layout(self):
        ...
        self._update_arrow(self._dir_cop, 0, 0.40, 'g')
        self._update_arrow(self._dir_grad, 0, 0.40, 'b')
        self._dir_txt_pzt = pg.TextItem("", anchor=(0, 1))
        self.p_dir.addItem(self._dir_txt_pzt)
        # 颜色图例（右上角）
        for label, color, y in [("Cal", "red", 1.15), ("COP", "green", 1.05), ("Grad", "blue", 0.95)]:
        ...
        self._update_arrow(self._mag_cop, 0, 0.08, 'g')
        self._update_arrow(self._mag_grad, 0, 0.08, 'b')
    ...
    def set_data(self, pzt_angle_deg, pzt_mag_val, force_angle_deg, force_mag_val,
                 ...
                 cop_state=0,
                 cop_angle_raw=0.0, cop_mag_raw=0.0,
                 grad_angle_raw=0.0, grad_mag_raw=0.0):
        with self.lock:
            ...
            self._cop_angle_raw = cop_angle_raw
            self._cop_mag_raw = cop_mag_raw
            self._grad_angle_raw = grad_angle_raw
            self._grad_mag_raw = grad_mag_raw
    ...
    def update_all(self):
        ...
        with self.lock:
            ...
            cop_angle_raw = self._cop_angle_raw; cop_mag_raw = self._cop_mag_raw
            grad_angle_raw = self._grad_angle_raw; grad_mag_raw = self._grad_mag_raw
        ...
        # Direction: fused=红, COP=绿, grad=蓝
        fs = self._font_size(12)
        self._update_arrow(self._dir_pzt, pzt_angle_deg, 0.45, 'r')
        self._update_arrow(self._dir_cop, cop_angle_raw, 0.40, 'g')
        self._update_arrow(self._dir_grad, grad_angle_raw, 0.40, 'b')
        self._dir_txt_pzt.setHtml(self._html(f'F:{pzt_angle_deg:.0f} C:{cop_angle_raw:.0f} G:{grad_angle_raw:.0f}', 'red', fs))
        self._dir_txt_pzt.setPos(0.75, 1.15)

        # Magnitude: fused=红, COP=绿, grad=蓝
        pzt_mag_len = max(min((pzt_mag_val / 5.0) * 0.65, 0.65), 0.01)
        cop_mag_len = max(min((cop_mag_raw / 5.0) * 0.65, 0.65), 0.01)
        grad_mag_len = max(min((grad_mag_raw / 500.0) * 0.65, 0.65), 0.01)
        self._update_arrow(self._mag_pzt, pzt_angle_deg, pzt_mag_len, 'r')
        self._update_arrow(self._mag_cop, cop_angle_raw, cop_mag_len, 'g')
        self._update_arrow(self._mag_grad, grad_angle_raw, grad_mag_len, 'b')
        self._mag_txt_pzt.setHtml(self._html(f'F:{pzt_mag_val:.0f} C:{cop_mag_raw:.0f} G:{grad_mag_raw:.0f}', 'red', fs))
