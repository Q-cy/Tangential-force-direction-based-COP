...
def _isolated_import_command() -> str:
    return (
        ...
        "assert [item.name for item in fields(TangentialFrame)] == ['base_data', 'adc_sum', 'cop_x', 'cop_y', 'angle', 'dx', 'dy', 'motion_state']; "
...
class DistributionConfigurationTests(unittest.TestCase):
    ...
    def test_pyproject_declares_public_distribution_contract(self):
        ...
        self.assertEqual(
            setuptools["package-data"],
            {
                ...
                "tangential.resources": ["fit_coefs.bin", "consistence_coeffs.npz"],
            },
        )
    ...
    def test_manifest_declares_exact_model_resource(self):
        ...
        self.assertIn("recursive-include src/tangential/resources *.npz", lines)
    ...
    def test_cython_module_manifest_has_eleven_sources_and_stubs(self):
        ...
        for module in COMPILED_MODULES:
            ...
            self.assertTrue(package_path.with_suffix(".py").is_file(), module)
            self.assertTrue(package_path.with_suffix(".pyi").is_file(), module)
            self.assertIn(f'"{module.replace("/", ".")}"', setup_text)
...
class WheelDistributionTests(unittest.TestCase):
    @unittest.skipUnless(
        (PACKAGE_ROOT / "resources" / "consistence_coeffs.npz").is_file(),
        "BLOCKED: default consistence_coeffs.npz source has not been authorized yet",
    )
    def test_wheel_build_and_isolated_install(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with zipfile.ZipFile(wheel_path) as archive:
                ...
                self.assertIn("tangential/resources/consistence_coeffs.npz", names)
