...
# python calibrate.py 1              # 自动找 data_1.csv
# python calibrate.py data_1.csv     # 自动补 SAVE_DIR
# python calibrate.py /full/path.csv # 完整路径
...
def data_loop():
    ...
    # 尝试加载标定系数
    cal_path = os.path.join(SAVE_DIR, "cal_coeffs.npz")
    cal_ready = False
    K, b = None, None
    if os.path.exists(cal_path):
        try:
            ...
            print(f"⚠️ 标定系数加载失败: {e}")
    else:
        print("💡 未找到标定文件。如需标定，先运行本程序采集CSV，然后执行：")
        print("   python /home/qcy/Project/code/Tangential/finger_tang_7_12/Cop/project/tang_7_12_Init_line_COP_vec_cal_realtime/calibrate.py /home/qcy/Project/data/2.PZT_tangential/weight/test/data_1.csv")
