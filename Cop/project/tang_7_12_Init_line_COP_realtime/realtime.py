...
class RealTimePlot:
    ...
    def build_layout(self):
        ...
        self.raw_force_line, = self.ax3b.plot([], [], 'r-', linewidth=1.5)
    ...
    def set_data(self, adc_angle, adc_mag, force_angle, force_mag, diff_frame, raw_adc_sum, raw_force_mag,
                 cop_x, cop_y, base_cop_x, base_cop_y, delta_cop_x, delta_cop_y):
        with self.lock:
    ...
    def update_raw_force(self):
        if len(self.raw_force_mag_history) > 0:
            xs = list(range(len(self.raw_force_mag_history)))
            ys = list(self.raw_force_mag_history)
            self.raw_force_line.set_data(xs, ys)
            self.ax3b.set_xlim(0, len(xs))
